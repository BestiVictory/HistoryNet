...
class MODEL():
    ...
    def __init__(self):
        ...
        # self.discriminator.load_weights("/home/ubuntu/mnt/LK/HistoryNet/MODEL/Reproducibility/Reproducibility_Parsing_01/my_model_discriminatorEpoch6.h5")
        self.discriminator.compile(loss=[wasserstein_loss],
                                   loss_weights=[1.0],
                                   optimizer=optimizer)  

        self.colorizationModel = self.colorization_model()
        # self.colorizationModel.load_weights("/home/ubuntu/mnt/LK/HistoryNet/MODEL/Reproducibility/Reproducibility_Parsing_01/my_model_colorizationEpoch6.h5")
        self.colorizationModel.compile(loss=['mse', 'kld', 'mse'],
        ...
        predAB, substracted, preseg = self.colorizationModel(img_L_3) 
        ...
        self.combined = Model(inputs=[img_L_3, img_L],
                              outputs=[predAB, substracted, discPredAB, preseg])
        self.combined.load_weights("/home/ubuntu/mnt/LK/HistoryNet/MODEL/Reproducibility/Reproducibility_Parsing_01/my_model_combinedEpoch6.h5")
        self.combined.compile(loss=['mse','kld', wasserstein_loss, 'mse'],
                              loss_weights=[2.0, 0.002, -0.1, 0.002],
    ...
    def colorization_model(self):
        ...
        segment_features =  keras.layers.UpSampling2D(size=(2,2))(segment_features)
        final_model = Model(input=input_img, outputs = [outputModel, global_featuresClass, segment_features])
        ...
        for epoch in range(config.NUM_EPOCHS):
                for batch in range(total_batch):
                    ...
                    g_loss =self.combined.train_on_batch([l_3, trainL],
                                                        [trainAB, predict, positive_y, segment])
                    # train discriminator
    ...
    def sample_images(self,test_data):
        ...
        for _ in range(total_batch):
                ...
                # predict AB channels
                predAB, ____, _____ = self.colorizationModel.predict(np.tile(testL,[1,1,1,3]))
...
if __name__ == '__main__':
    ...
    with open(os.path.join(log_path, str(datetime.datetime.now().strftime("%Y%m%d")) + "_" + str(config.BATCH_SIZE) + "_" + str(config.NUM_EPOCHS) + ".txt"), "w") as log:
        ...
        train_data = data.DATA(config.TRAIN_DATA)
        test_data = data.DATA(config.TEST_DATA)
