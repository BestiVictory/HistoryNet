...
class MODEL():
    ...
    def __init__(self):
        ...
        # self.colorizationModel.load_weights("/home/ubuntu/LiuKe/HistoryNet_Github/my_model_colorizationEpoch7.h5")
        self.colorizationModel.compile(loss=['mse', 'kld', 'kld'],
        ...
        predAB, substracted, info = self.colorizationModel(img_L_3) 
        ...
        self.combined = Model(inputs=[img_L_3, img_L],
                              outputs=[predAB, substracted, discPredAB, cinfo, info])      
        self.combined.compile(loss=['mse','kld', wasserstein_loss, 'kld', 'kld'],
                              loss_weights=[2.0, 0.002, -0.1, 0.002, 0.0],
                              optimizer=optimizer) # 1/300

        self.log_path= config.LOG_DIR
        self.callback = TensorBoard(self.log_path)
        self.callback.set_model(self.combined)
        self.train_names = ['loss', 'mse_loss', 'kullback_loss', 'wasserstein_loss', 'mse_loss_2', 'kld_loss_2']
    ...
    def colorization_model(self):
        ...
        final_model = Model(input=input_img, outputs = [outputModel, global_featuresClass, info_features])
        return final_model
    ...
    def train(self, data,test_data, log,sample_interval=1):
        ...
        for epoch in range(config.NUM_EPOCHS):
                for batch in range(total_batch):
                    # new batch
                    trainL, trainAB, _, predict = data.generate_batch()
                    l_3=np.tile(trainL,[1,1,1,3])      
               
                    _, __, info= self.colorizationModel.predict(l_3)
                    
                    # train generator
                    g_loss =self.combined.train_on_batch([l_3, trainL],
                                                        [trainAB, predict, positive_y, info, info])
                    # train discriminator
